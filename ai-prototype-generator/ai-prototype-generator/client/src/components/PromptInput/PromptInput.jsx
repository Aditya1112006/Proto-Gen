...
function PromptInput({ onSubmit, isLoading, mode, setMode, counters = {}, domainChanged = false, oldDomain = null }) {
  ...
  useEffect(() => {
    if (domainChanged) {
      console.log('[PromptInput] Domain changed, showing alert')
  ...
  // Clear prompt AFTER successful submission (isLoading transitions from true to false)
  // BUT only if domain didn't change (keep prompt if domain changed for retry)
  useEffect(() => {
    // Only clear when isLoading transitions from true to false (submission complete)
    if (prevIsLoadingRef.current === true && isLoading === false) {
      console.log('[PromptInput] Submission complete, domainChanged:', domainChanged)
      // Don't clear if domain changed - user might want to see what they typed
      if (!domainChanged) {
        setPrompt('')
      }
    }
    prevIsLoadingRef.current = isLoading
  }, [isLoading, domainChanged])

  const handleSubmit = (e) => {
    e.preventDefault()
    if (prompt.trim() && !isLoading) {
      console.log('[PromptInput] Submitting prompt:', prompt.trim().substring(0, 50))
      // Don't clear here - let the effect handle it after submission completes
        ...
        <div className="bg-amber-50 border-b border-amber-200 p-4 flex items-start gap-3 transition-all">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-amber-700 mt-1">
              Your prompt was different from <strong>"{oldDomain || 'previous'}"</strong>. A new prototype has been generated.
            </p>
      ...
      <form onSubmit={handleSubmit} className="p-4">
        <div className="relative">
          <textarea
            value={prompt}
            onChange={(e) => {
              // This should work now - just update the state
